...
function isRefundOrder(order: {
    ...
    has_refund?: number | boolean | null
    refund_status?: string | null
}): boolean {
    const status = Number(order.status || 0)
    return status === 8
        || status === 12
        || Number(order.has_refund || 0) === 1
        || Boolean(order.refund_status?.trim())
        || Boolean(order.status_text?.includes('退款'))
}
...
function getOrderRefundAmount(order: {
    ...
    if (!isRefundOrder(order)) return 0
    if (Number(order.status || 0) === 8) return revenue

    const refundAmount = parseFloat(order.refund_amount || '0') || 0
    return refundAmount > 0 && refundAmount <= revenue ? refundAmount : revenue
}
...
export function getRevenueReport(period: 'day' | 'week' | 'month' = 'day', days = 30, startDate?: string, endDate?: string, groupId?: number) {
    ...
    const orders = db.prepare(`
        SELECT ${REVENUE_TIME_EXPR} AS report_time,
               ...
               pay_time, ship_time, complete_time, refund_amount, refund_status, has_refund
        FROM orders
    ...
    for (const o of orders) {
        ...
        const refundAmt = getOrderRefundAmount(o, revenue)
        ...
        if (o.status === 4) {
            ...
            dayData.netIncome += Math.max(dayPaid - refundAmt, 0)
        }
        ...
        if (o.status === 4) { const paid = getOrderPaidAmount(o); itemData.netIncome += Math.max(paid - refundAmt, 0) }
        ...
        if (o.status === 4) { const paid = getOrderPaidAmount(o); accData.netIncome += Math.max(paid - refundAmt, 0) }
        ...
        if (o.status === 4) {
            ...
            netIncome += Math.max(successPaid - refundAmt, 0)
        }
