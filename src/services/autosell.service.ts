...
const orderDeliveryLocks = new Map<string, Promise<void>>()
...
async function withOrderDeliveryLock<T>(orderId: string, task: () => Promise<T>): Promise<T> {
    const previous = orderDeliveryLocks.get(orderId) || Promise.resolve()
    ...
    orderDeliveryLocks.set(orderId, tail)
    ...
    try {
        ...
        releaseCurrent()
        if (orderDeliveryLocks.get(orderId) === tail) {
            orderDeliveryLocks.delete(orderId)
        }
    }
}
...
/**
 * 串行完成单个订单的内容生成、消息发送和成功日志写入。
 */
export async function processAndSendAutoSell(
    params: ProcessAutoSellParams,
    sendContent: (content: string) => Promise<boolean>
): Promise<ProcessAutoSellResult> {
    return withOrderDeliveryLock(params.orderId, async () => {
        const result = await processAutoSell(params)
        ...
        let sent = false
        try {
            sent = await sendContent(result.content)
        } catch (error) {
            logger.error(`订单 ${params.orderId} 发送发货消息异常: ${error}`)
        }

        if (!sent) {
            rollbackAutoSellReservation(params.orderId, result)
            recordAutoSellDeliveryLog({
