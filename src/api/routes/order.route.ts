...
import { processAndSendAutoSell } from '../../services/autosell.service.js'
import {
...
import type { OrderRecord } from '../../types/order.types.js'
...
function getRedeliveryBlockReason(order: OrderRecord): string | null {
    const refundAmount = Number(order.refundAmount || 0)
    const hasRefund = order.hasRefund
        || order.status === 8
        || order.status === 12
        || Boolean(order.refundStatus?.trim())
        || Boolean(order.statusText?.includes('退款'))
        || (Number.isFinite(refundAmount) && refundAmount > 0)

    if (hasRefund) {
        return '退款或售后订单不能自动补发，请人工核对'
    }

    if (![OrderStatus.PENDING_SHIPMENT, OrderStatus.PENDING_RECEIPT, OrderStatus.COMPLETED].includes(order.status)) {
        return '仅已付款、待收货或已完成订单可以补发'
    }
...
export function createOrderRoutes(getClientManager: () => ClientManager | null) {
    ...
    app.post('/:orderId/redeliver-missing', async (c) => {
        ...
        const detail = await fetchAndUpdateOrderDetail(client, orderId, { triggerAutoSell: false })
        if (!detail) {
            return c.json({ error: '刷新订单详情失败，未执行补发' }, 500)
        }
        ...
        const blockReason = getRedeliveryBlockReason(refreshedOrder)
        if (blockReason) {
            return c.json({ error: blockReason }, 400)
        }
        ...
        const result = await processAndSendAutoSell(
            ...
            content => client.sendMessage(refreshedOrder.chatId!, refreshedOrder.buyerUserId!, content)
        )
